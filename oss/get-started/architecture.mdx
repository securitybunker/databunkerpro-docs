...
![Databunker solution](/oss/get-started/databunker-solution.png)
...
1. Duplicate record validation using hashed values of email address, phone number, login name
...
After creating a new user record, Databunker returns to the caller a user token in UUID format. That token can be saved in existing database instead of storing personal records or PII/PHI.

![Databunker request flow](/oss/get-started/databunker-request-flow.png)
