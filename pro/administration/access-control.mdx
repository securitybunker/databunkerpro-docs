...
## How CRBAC differs from plain RBAC

Static RBAC answers "which endpoints may this role call?". CRBAC additionally evaluates the request itself, so the same role can be allowed or refused depending on runtime context passed in `request_metadata`, and the response can be masked field by field rather than allowed or denied whole.

That is what makes it usable for regulated data: a support agent can hold one role and still see different fields depending on which record they are looking at, without you minting a role per situation.

## Next steps

- [Multi-tenancy](/pro/administration/multi-tenancy) — the database-level boundary policies operate inside
- [Sub-accounts](/pro/concepts/sub-accounts) — choosing between tenants and groups
- [Bulk export controls](/pro/concepts/select-security) — how policies gate bulk retrieval
- [Authentication](/pro/api/authentication) — issuing the scoped tokens a policy applies to
