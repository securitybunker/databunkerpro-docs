...
In case the current Wrapping Key is lost or compromised, Databunker Pro allows for recovery using [Shamir Key Shares](/pro/administration/shamir-keys). This ensures that the Master Key can be safely re-encrypted with a new Wrapping Key without exposure.
