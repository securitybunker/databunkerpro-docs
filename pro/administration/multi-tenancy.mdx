...
**Note:** Multi-tenancy is supported on PostgreSQL, MySQL, and Oracle. On PostgreSQL it is enforced with native row-level security (RLS) — see [Database-level tenant isolation](#database-level-tenant-isolation).
...
On PostgreSQL, tenant isolation is enforced by the database itself rather than by the correctness of the queries Databunker Pro issues. Two dedicated roles are created during setup, both `NOSUPERUSER NOLOGIN` — neither can log into the database directly, and both are reachable only through `SET ROLE` inside Databunker Pro's own session.
...
| `madmin` | `BYPASSRLS`, granted `SELECT` on the `users` table only | The cross-tenant System Operations below |

### How mtenant scoping works

Every tenant-scoped table has row-level security enabled, with a policy tying each row to the tenant set on the current session:
...
From that point the database filters rows itself, so a query that omits its tenant condition — or gets it wrong — still cannot return another tenant's records. This is defence in depth: isolation does not depend on every query being written correctly. Roughly two dozen tables are covered, including `users`, `userapps`, `audit`, `sessions`, `agreements`, `files`, `tokens`, and the versioning tables.
...
<Note>
The `xtokens` table is intentionally **not** under RLS. An incoming API token must be resolved to a tenant before the tenant is known, so that lookup cannot itself be tenant-scoped. Tokens carry their tenant binding, which is then applied to every subsequent query in the request.
</Note>

### How madmin is constrained
...
Multi-tenancy is supported on both, but neither uses a database-enforced equivalent of RLS: there, tenant scoping is applied in the queries Databunker Pro issues. The isolation model is identical from the API's point of view, without this second, database-level line of defence.
...
- [Access control](/pro/administration/access-control) — policies and roles within a tenant
- [Sub-accounts](/pro/concepts/sub-accounts) — choosing between tenants and groups
- [Multi-jurisdiction deployment](/pro/concepts/global-deployment) — when a separate instance beats a separate tenant
