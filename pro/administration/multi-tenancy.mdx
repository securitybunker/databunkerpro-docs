...
**Note:** Multi-tenancy is supported on PostgreSQL, MySQL, and Oracle. On PostgreSQL it is enforced with native row-level security (RLS), using a restricted `mtenant` role for all ordinary traffic and a narrowly-granted `madmin` role for the few deliberate cross-tenant operations — see [Database-level tenant isolation](/pro/get-started/security-overview#database-level-tenant-isolation).
