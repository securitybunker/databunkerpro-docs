...
sidebarTitle: Unattended install
...
After a successful `/autoinstall`, the setup endpoint shuts down and the container **restarts in normal operational mode**. The step is **idempotent**: if the database is already installed, skip it.
...
A response of `{"status":"ok","installed":false}` means setup is needed. If `installed` is `true` (or the endpoint is gone once running normally), the vault is already installed — **stop here**.
</Step>
...
# 2. install only if not already installed (idempotent)
if requests.get(f"{BASE}/dbstatus", timeout=10).json().get("installed") is False:
