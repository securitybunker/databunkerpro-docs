...
PGSQL_PORT=5432
PGSQL_USER_NAME=bunkeruser
PGSQL_USER_PASS=<database password>
...
<Warning>
`DATABUNKER_SETUPKEY` only matters during the install, which is a one-time operation: anyone who reaches the service with it can install the vault, so use a long random value. Once the vault is installed the setup endpoint is disabled and the key stops being sensitive — you can safely remove it.
</Warning>
...
- Learn how to issue scoped credentials in the [Authentication reference](/pro/api/authentication).
