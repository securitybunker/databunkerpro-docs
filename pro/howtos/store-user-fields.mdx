...
- **Erasure is yours to manage.** The `tokens` table is not keyed by user token, so `UserDelete` does not touch these tokens and a data-subject erasure request will not reach them. You must record which tokens belong to which user and delete them yourself with `TokenDelete` or `BulkDeleteTokens` — or give each token an expiry, as below.
...
| Erasure | `UserDelete`, or `finaltime` / `slidingtime` at creation | `TokenDelete` / `BulkDeleteTokens` on tokens you tracked yourself, or `finaltime` / `slidingtime` at creation |
