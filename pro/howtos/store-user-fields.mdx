...
- **One licensed record** per user, however many fields it holds. A card stored inside the profile is not metered separately.
...
- Repeated values are not deduplicated. Two users at the same address store that address twice.

## Option 2 — Tokenize individual fields

`TokenCreate` tokenizes a single value. It has no connection to any user record.
...
`tokenuuid` addresses the token; `tokenbase` is the format-preserving form — same length, still Luhn-valid, so it drops into a column that already validates card numbers. Either value detokenizes through `TokenGet`.
...
<Note>
Only `creditcard`, `unixtimestamp`, `uint32` and `uint64` produce a format-preserving `tokenbase`. The remaining types — `text` (alias: `string`), `email`, `ssn` and `uuid` — return a `tokenuuid` with an empty `tokenbase`. If you are tokenizing arbitrary strings, expect a UUID, not a look-alike value.
</Note>
...
<Note>
`X-Bunker-Tenant` is still required — without it the request cannot be routed and every UUID returns 404. That is addressing, not authentication.
</Note>
...
| Shared records, versioning, agreements, audit | yes | no |
| Deduplicate repeated values | no | yes, with `unique` |
| Format-preserving output | no | 4 of 8 token types |
