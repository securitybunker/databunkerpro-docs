...
| `files` | `token` | Encrypted file metadata and wrapped per-file keys |
| `filetags` | `token` | File tags |
...
Clearing `files` removes the metadata and the encryption keys, but not the stored objects themselves — see [Stored file objects](#stored-file-objects) below.
...
TRUNCATE TABLE filetags RESTART IDENTITY;
...
The `files` table holds the metadata and the per-file key; the encrypted file itself lives in the configured [object storage](/pro/concepts/file-vault) backend — local disk, Amazon S3, Google Cloud Storage, or Azure Blob Storage — under `tenant/<tenantid>/<usertoken>/<fileuuid>`.
...
```bash
# local backend, using the object_storage.local_dir from databunker.yaml
rm -rf /databunker/files/tenant/*
```

```bash
# Amazon S3
aws s3 rm "s3://<your-bucket>/tenant/" --recursive
```
