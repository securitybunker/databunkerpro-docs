...
To try these endpoints against a live instance, start one with [demo mode](/pro/get-started/quickstart) — one `docker run`, root token `DEMO`.
...
### Multi-Tenant Usage
Multi-tenancy is supported on PostgreSQL, MySQL (Percona), and Oracle backend databases. On PostgreSQL it is enforced with the database's native row-level security (RLS).
