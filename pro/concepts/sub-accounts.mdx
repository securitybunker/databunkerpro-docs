...
The tenant admin token is one of several credential types — see the [Authentication reference](/pro/api/authentication) for the full hierarchy.
...
| | Multi-tenancy | Groups with CRBAC |
| --- | --- | --- |
| Isolation | Enforced by the database (PostgreSQL RLS) | Enforced by policy evaluation |
| Administration | Each tenant administered independently | Central administration |
| Best for | Separate customers or jurisdictions | Hierarchy and roles inside one customer |

The two combine: multi-tenancy for the database-level boundary, groups for fine-grained permissions within it.

## Next steps

- [Multi-tenancy](/pro/administration/multi-tenancy) — tenant API and database-level isolation
- [Access control](/pro/administration/access-control) — policies, roles, and field masking
