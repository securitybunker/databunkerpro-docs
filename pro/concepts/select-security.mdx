...
1. **Temporary Token:** The unlock request generates a temporary token with a short expiration window (60 seconds).
...
- It expires automatically after 60 seconds
...
- [Access control](/pro/administration/access-control) — the policy engine that governs bulk operations
- [Authentication](/pro/api/authentication#bulk-unlock-uuid) — how the unlock UUID is issued and its 60-second lifetime
- [Backup and recovery](/pro/administration/backup-and-recovery#exporting-every-record) — the sanctioned full-export path
