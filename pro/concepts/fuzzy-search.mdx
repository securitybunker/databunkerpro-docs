...
Fuzzy search is **not covered by the published [benchmarks](/pro/get-started/performance)**, which measure the exact-match write and detokenisation paths only. Unlike a lookup by token or email — a hashed-index point read at ~14 ms — approximate matching cannot use those indexes, so cost grows with the number of records examined. Restrict it to administrative interfaces and measure it against your own data before putting it on a user-facing path.

## Next steps

- [Partial search](/pro/concepts/partial-search) — prefix and substring matching on indexed fields
- [Access control](/pro/administration/access-control) — restrict who may run a search and which fields come back
- [Bulk export controls](/pro/concepts/select-security) — why enumeration is default-deny
- [SDKs](/pro/developer-tools/overview) — JavaScript and Python clients
