...
Because records are encrypted and indexes are hashed, true substring search is not possible. [Fuzzy search](/pro/concepts/fuzzy-search) covers approximate matching on values you already know, and the user group API lets you build exact-match indexes for the attributes you care about.

- [Fuzzy search](/pro/concepts/fuzzy-search) — approximate matching
- [Access control](/pro/administration/access-control) — who may search, and which fields are returned
- [Security overview](/pro/get-started/security-overview) — why the indexes are hashed
