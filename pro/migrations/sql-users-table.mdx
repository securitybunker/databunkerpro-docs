...
CREATE TABLE newusers (
    userid    BIGINT PRIMARY KEY,   -- same value as users.userid
    usertoken UUID   NOT NULL UNIQUE
);
```

`userid` is copied from `users.userid`, so no generator is needed for the backfill itself.
...
```sql
CREATE TABLE newusers (
    userid    BIGSERIAL PRIMARY KEY,   -- same value as users.userid
    usertoken UUID      NOT NULL UNIQUE
);
```

Use `SERIAL` if your existing `userid` is a 32-bit `INTEGER`.
...
MySQL has no native UUID column — `CHAR(36)` stores the token as returned.

</Tab>
</Tabs>

<Warning>
**On PostgreSQL, reset the sequence before you cut over.** Explicit id inserts do not advance a `SERIAL` sequence, so it is still at 1 while your ids run into the millions, and the first insert after the swap collides with an existing row:

```sql
SELECT setval(pg_get_serial_sequence('newusers', 'userid'),
              (SELECT max(userid) FROM newusers));
```

MySQL needs no equivalent — `AUTO_INCREMENT` advances itself when you insert a higher explicit value.
</Warning>
...
- **A re-run cannot create duplicate tokens.** A second attempt for the same `userid` is rejected with `Duplicate custom value` instead of silently minting a second token.
- **You can look a user up by `userid` later** with `UserGet` using `mode=custom`.
...
The script counts the rows, walks the table in chunks of 4,000, sends each chunk to `UserCreateBulk`, and writes `userid` plus the returned token into `newusers`. Failures are written to a CSV for review.

It is written for PostgreSQL. On MySQL, swap `psycopg2` for your connector and `ON CONFLICT (userid) DO NOTHING` for `INSERT IGNORE`.
...
api = DatabunkerproAPI(
    os.environ["DATABUNKER_API_URL"],       # e.g. https://databunkerpro.myorg.com
...
def fetch_chunk(offset):
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT * FROM users ORDER BY userid LIMIT %s OFFSET %s",
            (CHUNK, offset),
        )
        return cur.fetchall()
...
def to_profile(row):
    """The whole row becomes the vault profile; 'custom' carries userid for lookups."""
    profile = {k: v for k, v in row.items() if v is not None}
    profile["custom"] = str(row["userid"])
    return profile
...
def save_tokens(created):
    ...
    with conn.cursor() as cur:
        psycopg2.extras.execute_values(
            cur,
            "INSERT INTO newusers (userid, usertoken) VALUES %s "
...
If the counts do not match, some rows reached the vault but their tokens were never written locally — the script died between the two writes. Because `custom` holds the legacy id, those tokens can be looked up again and written into `newusers`:

```python
# reconcile.py — recover tokens for rows missing from newusers.
# Reuses the imports, `api`, and `conn` from the backfill script above.
CHUNK = 4000

with conn.cursor() as cur:
    cur.execute("SELECT userid FROM users WHERE userid NOT IN (SELECT userid FROM newusers)")
    missing = [r[0] for r in cur.fetchall()]

print(f"{len(missing)} users without a token")
recovered = 0

for offset in range(0, len(missing), CHUNK):
    batch = missing[offset:offset + CHUNK]

    # The unlock UUID is short-lived, so take a fresh one for each chunk.
    unlock = api.bulk_list_unlock()["unlockuuid"]
    found = api.bulk_list_users(
        unlock, [{"mode": "custom", "identity": str(uid)} for uid in batch]
    )
    ...
    recovered += len(pairs)
    print(f"  {offset + len(batch)}/{len(missing)} checked, {recovered} recovered",
          flush=True)

print(f"\n{len(missing)} missing, {recovered} recovered, "
      f"{len(missing) - recovered} never reached the vault")
```

`BulkListUsers` is not paginated — every criterion you send is evaluated — so the chunking here is about keeping request bodies and unlock lifetimes sane, not about paging through results. Ids with no vault record are simply absent from `rows`: those never reached the vault, so find them in `migration-errors.csv` and resolve the conflict there.

Re-running is safe. Each pass recomputes `missing` from the two tables, so recovered rows drop out and only genuine gaps remain.

<Note>
This call requires the `list_users` configuration flag to be enabled, otherwise it returns `403 BulkListUsers is disabled`.
</Note>
...
Once step 5 has run clean. Both renames must land together, or requests arriving in between hit a missing table.

<Tabs>
<Tab title="PostgreSQL">

DDL is transactional, so both renames commit as one and the cutover is effectively instant:
...
DDL auto-commits, so a transaction gives you nothing here. Use `RENAME TABLE`, which swaps both in a single atomic statement:

```sql
RENAME TABLE users TO users_old, newusers TO users;
```
...
If other tables have foreign keys pointing at it, the drop is refused until those constraints are dropped or repointed at the new `users`.

<Warning>
**Foreign keys follow the table, not the name.** Any table with a foreign key to `users(userid)` now references `users_old`, because constraints track the table itself rather than its name. Check before swapping, and recreate the constraints against the new `users` afterwards:
