...
- **Writes reach ~40% of PostgreSQL's ceiling.** Oracle sustained **~5,100 rec/s**. The main reason is that **PostgreSQL's SQL supports a more efficient way to insert many records at once**: PostgreSQL writes a whole batch as a single, set-based `INSERT` statement, whereas Oracle has no equivalent multi-row `INSERT` syntax and must use the heavier `INSERT ALL` form, which does more work per row. That extra per-row work made the Oracle load **disk-IOPS-bound** — write IOPS ran near the 12,000 provisioned cap while CPU sat at ~18%. And unlike PostgreSQL, which is *app*-bound at 10 M and scales toward ~12,000 rec/s by adding stateless app instances, Oracle was already **database-bound on one instance**; pushing past it needs **more provisioned IOPS**, not more workers (past 16 workers latency rose without throughput).
