...
- **Writes reach ~40% of PostgreSQL's ceiling.** Oracle sustained **~5,100 rec/s** — mainly because **PostgreSQL's multi-row `INSERT` is more efficient than Oracle's `INSERT ALL`** for writing a batch. The Oracle load was **disk-IOPS-bound** — write IOPS ran near the 12,000 provisioned cap while CPU sat at ~18%. Unlike PostgreSQL, which is *app*-bound at 10 M and scales toward ~12,000 rec/s by adding stateless app instances, Oracle was already **database-bound on one instance**; pushing past it needs **more provisioned IOPS**, not more workers.
