...
- **Running on Oracle instead?** A comparable 10 M run on RDS Oracle SE2 is included below: similar (slightly faster) read latency, about **40% of PostgreSQL's write ceiling** (`INSERT ALL` overhead), and roughly **3× the on-disk size** (`CLOB` storage). See [Oracle Database backend (10 M)](#oracle-database-backend-10-m).
...
- **Writes reach ~40% of PostgreSQL's ceiling.** Oracle sustained **~5,100 rec/s**. Oracle has no multi-row `INSERT`, so Databunker Pro rewrites each bulk batch as `INSERT ALL`, which is heavier per row and here became **disk-IOPS-bound** — write IOPS ran near the 12,000 provisioned cap while CPU sat at ~18%. Unlike PostgreSQL, which is *app*-bound at 10 M and scales toward ~12,000 rec/s by adding stateless app instances, Oracle was already **database-bound on one instance**; pushing past it needs **more provisioned IOPS**, not more workers (past 16 workers latency rose without throughput).
