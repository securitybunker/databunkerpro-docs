...
- **At peak, Oracle writes are about 2.4× slower than PostgreSQL** (~5,100 rec/s vs ~12,000) — mainly because **PostgreSQL's multi-row `INSERT` is more efficient than Oracle's `INSERT ALL`** for writing a batch. On a single instance the two are close (~5,100 vs PostgreSQL's ~5,800 here), but Oracle's load was already **disk-IOPS-bound** (write IOPS near the 12,000 cap while CPU sat at ~18%), so it stays around 5,100 rec/s — whereas PostgreSQL is *app*-bound at 10 M and scales toward ~12,000 rec/s by adding stateless app instances. Pushing Oracle higher needs **more provisioned IOPS**, not more workers.
