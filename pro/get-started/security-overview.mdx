...
Databunker Pro implements [multi-tenancy](/pro/administration/multi-tenancy) on PostgreSQL, MySQL, and Oracle. Queries executed by specific tenants are restricted to their own records. On PostgreSQL, isolation is enforced with the database's native row-level security (RLS) mechanism.
