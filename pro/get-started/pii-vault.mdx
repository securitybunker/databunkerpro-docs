...
The PII Vault stores and tokenizes personal data (PII, PHI, KYC, PCI) so it never sits in your application database.
...
Together, these properties mean that **tokens within a tenant are stable, deterministic join keys** — SQL queries that previously joined on `email` can join on `user_token` with no round-trip to the vault.
...
Records are encrypted with AES-256, access is governed by [role-based policies](/pro/administration/access-control), and every request is written to the [audit trail](/pro/get-started/security-overview). Tenants are isolated at the database level on PostgreSQL, and the application tier scales horizontally. See the [Security overview](/pro/get-started/security-overview) for the full control set.

## Next steps

- [Quickstart](/pro/get-started/quickstart) — run a vault and store your first record in one command
- [Search the vault](/pro/concepts/fuzzy-search) — exact, fuzzy, and partial lookups over encrypted records
- [Tokenize credit cards](/pro/concepts/tokenization) — format-preserving tokens for PCI data
- [Performance and sizing](/pro/get-started/performance) — measured throughput and instance sizing by record count
